...
PALETTES = {
    ...
    # The heart, as the HUD has always drawn it: an outline, a body, and a
    # shine in the upper-left lobe. Lifted here from ui.cpp so the bar and the
    # thing lying on the floor are one picture rather than two that have to be
    # kept in step by hand.
    "heart": [
        ("#", ( 42,  10,  16)),   # outline
        ("a", (128,  22,  34)),   # body, shadow
        ("b", (196,  32,  46)),   # body
        ("c", (232,  74,  84)),   # body, lit
        ("d", (255, 158, 162)),   # shine
    ],
...
# Each mob is drawn from three views -- front, side and back -- and the side
# view is MIRRORED by the renderer for the far side, so it is drawn once. The
# attack poses are FRONT only: a mob swinging at you is looking at you by
# construction, so a side-facing attack frame could never be drawn.
#
# Frames are laid out view-major -- all of FRONT, then all of SIDE, then all of
# BACK, then the attack poses -- and VIEWS/WALK below is what render.cpp indexes
# that layout with. Kept as named per-view lists and concatenated rather than as
# one flat list of eight strings, so a diff on the art says which view moved.
...
ZOMBIE = ZOMBIE_FRONT + ZOMBIE_SIDE + ZOMBIE_BACK + ZOMBIE_ATTACK
...
CREEPER = CREEPER_FRONT + CREEPER_SIDE + CREEPER_BACK + CREEPER_ATTACK
...
SKELETON = SKELETON_FRONT + SKELETON_SIDE + SKELETON_BACK + SKELETON_ATTACK
...
# How many views each family is drawn from, and how long one view's walk cycle
# is. Frames are view-major, so render.cpp finds view V's walk frame N at
...
def emit(path):
    ...
    for fam in ["zombie", "creeper", "skeleton", "pick", "sword", "hand", "heart"]:
        pal = PALETTES[fam]
    ...
    for fam in MOB_ORDER:
        ...
        n = len(SPRITES[fam])
        if WALK[fam] < 1:
            sys.exit(f"{fam}: WALK is {WALK[fam]}, and render.cpp takes a "
                     f"modulo by it to pick a stride frame")
        need = VIEWS[fam] * WALK[fam]
        if n < need:
            sys.exit(f"{fam}: {VIEWS[fam]} views x {WALK[fam]} walk frames "
                     f"needs {need} frames, but only {n} are drawn")
        up, cap = fam.upper(), fam.capitalize()
        parts.append(
            f"  {{ &k{cap}[0][0][0], k{cap}Pal, {up}_W, {up}_H, "
            f"{up}_FRAMES, {up}_COLOURS, {VIEWS[fam]}, {WALK[fam]}, "
            f"{heights[fam]:.2f}f }},\n")
    parts.append("};\n\n}  // namespace sprites\n")
